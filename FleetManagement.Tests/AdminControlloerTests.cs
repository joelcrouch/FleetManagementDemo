...
//            _mockConfig = new Mock<IConfiguration>();
//            _mockLogger = new Mock<ILogger<AdminController>>();
//            //_mockTelemetry = new Mock<TelemetryClient>();
//            var _mockTelemetry = new TelemetryClient();  //real
//        }
...
//            var controller = new AdminController(_mockConfig.Object, _mockLogger.Object, _mockTelemetry.Object);
...
//            // Assert
//            var okResult = Assert.IsType<OkObjectResult>(result);
//            var data = Assert.IsType<Dictionary<string, object>>(okResult.Value);

//            // There should be 3 diagnostic result sets (even if empty)
//            Assert.True(data.ContainsKey("CurrentlyExecuting"));
//            Assert.True(data.ContainsKey("DatabaseSize"));
//            Assert.True(data.ContainsKey("ActiveConnections"));
//            _mockTelemetry.Verify(t => t.TrackEvent("DatabaseHealthCheckStarted", null, null), Times.Once);

//            //_mockTelemetry.Verify(t => t.TrackEvent("DatabaseHealthCheckStarted"), Times.Once);
//        }
...
//            // Arrange
//            _mockConfig.Setup(c => c.GetConnectionString("DefaultConnection"))
//                       .Returns("Server=invalid;Database=fake;User Id=fake;Password=fake;");
...
//            var errorData = errorResult.Value as dynamic;
//            Assert.NotNull(errorData);
//            Assert.Contains("Error retrieving database diagnostics", (string)errorData.message);
//            _mockTelemetry.Verify(t => t.TrackEvent("DatabaseHealthCheckStarted", null, null), Times.Once);
//            //_mockTelemetry.Verify(t => t.TrackException(It.IsAny<Exception>()), Times.Once);
//        }
